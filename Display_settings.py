...
class DisplayModeSelector:
    ...
    def __init__(self, WIDTH, HEIGHT, screen, small_font, flame_colors):
        ...
        self.instruction_surface = self.small_font.render(
            "Choose Display Size:", True, (255, 255, 255)
        )
    ...
    def update_particles(self, delta_time):
        ...
        for particle in self.particles:
            ...
            particle["x"] = (
                self.WIDTH // 2 + math.cos(particle["angle"]) * particle["orbit_distance"]
            )
            particle["y"] = (
                self.HEIGHT // 2 + math.sin(particle["angle"]) * particle["orbit_distance"]
            )
    ...
    def update_color_transition(self, delta_time):
        ...
        if self.color_transition >= 1.0:
            ...
            self.next_color = random.choice(
                [c for c in self.flame_colors if c != self.current_color]
            )
    ...
    def get_title_color(self):
        ...
        r = int(
            self.current_color[0] * (1 - self.color_transition)
            + self.next_color[0] * self.color_transition
        )
        g = int(
            self.current_color[1] * (1 - self.color_transition)
            + self.next_color[1] * self.color_transition
        )
        b = int(
            self.current_color[2] * (1 - self.color_transition)
            + self.next_color[2] * self.color_transition
        )
        return (r, g, b)
    ...
    def draw_title(self, title_color):
        ...
        title_rect_center = (
            self.WIDTH // 2,
            self.HEIGHT // 2 - self.title_offset + self.title_offset_y,
        )
        ...
        # Draw shadow
        shadow_rect = cached_title["shadow"].get_rect(
            center=(title_rect_center[0] + 1, title_rect_center[1] + 1)
        )
        self.screen.blit(cached_title["shadow"], shadow_rect)

        # Add dynamic glow (simplified - only 2 glow layers instead of 8)
        glow_rect = cached_title["glow1"].get_rect(
            center=(title_rect_center[0] - 1, title_rect_center[1] - 1)
        )
        ...
        highlight_rect = cached_title["highlight"].get_rect(
            center=(title_rect_center[0] - 4, title_rect_center[1] - 4)
        )
        self.screen.blit(cached_title["highlight"], highlight_rect)

        mid_rect = cached_title["mid_tone"].get_rect(
            center=(title_rect_center[0] + 2, title_rect_center[1] + 2)
        )
        self.screen.blit(cached_title["mid_tone"], mid_rect)

        inner_shadow_rect = cached_title["inner_shadow"].get_rect(
            center=(title_rect_center[0] + 4, title_rect_center[1] + 4)
        )
    ...
    def draw_instructions(self):
        ...
        display_rect = self.instruction_surface.get_rect(
            center=(self.WIDTH // 2, self.HEIGHT // 2 + self.instruction_y_pos)
        )
    ...
    def draw_buttons(self, default_hover, qboard_hover):
        ...
        for i in range(1, glow_intensity):
            ...
            alpha_factor = (1 - i / glow_intensity) * multiplier
            glow_color = (
                min(int(255 * multiplier * alpha_factor), 255),
                0,
                min(int(150 * multiplier * alpha_factor), 255),
            )
    ...
    def draw_auto_detected_indicator(self, current_time, default_hover, qboard_hover):
        ...
        auto_text = self.small_font.render(
            f"Auto-detected: {self.detected_display}", True, auto_text_color
        )
        auto_rect = auto_text.get_rect(
            center=(self.WIDTH // 2, self.HEIGHT // 2 + self.button_y_pos + self.button_height + 30)
        )
    ...
    def draw_collaboration_text(self, current_time):
        ...
        sangsom_color = tuple(
            int(bright_yellow[i] * (1 - pulse_factor) + lite_yellow[i] * pulse_factor)
            for i in range(3)
        )
        ...
        collab_rect2 = collab_text2.get_rect(
    ...
    def draw_creator_text(self, current_time):
        ...
        creator_text = self.small_font.render(
            "Created by Teacher Evan and Teacher Lee", True, (255, 255, 255)
        )
        creator_rect = creator_text.get_rect(
            center=(self.WIDTH // 2, self.HEIGHT - 40 + creator_float)
        )
