...
from settings import BLACK, FLAME_COLORS
from utils.animation_system import (
    AnimatedTarget,
    EaseType,
...
class EnhancedCenterPieceManager:
    ...
    def set_animation_settings(
        self, enabled: bool = True, entrance_type: str = "scale", continuous: bool = True
    ):
        """
    ...
    def update_target(self, target_letter: str, mode: str):
        ...
        if target_letter != self.target_text or mode != self.target_mode:
            ...
            if self.current_target and self.animation_enabled:
                self.animation_manager.animate_target_exit(self.current_target, "fade")
    ...
    def update_and_draw(
        ...
        if mode == "shapes":
            self._draw_shape_target(
                screen, target_letter, self.player_x + offset_x, self.player_y + offset_y
            )
    ...
    def _draw_center_target_fallback(
        self, screen: pygame.Surface, target_letter: str, mode: str, offset_x: int, offset_y: int
    ):
    ...
    def _draw_shape_target(
        self, screen: pygame.Surface, target_letter: str, center_x: int, center_y: int
    ):
