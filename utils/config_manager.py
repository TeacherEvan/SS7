...
class ConfigurationManager:
    ...
    def _load_all_configs(self):
        ...
        try:
            ...
            self._config_cache["game"] = self._load_json_config(
                self.game_config_path, self._get_default_game_config()
            )

            # Load teacher customization configuration
            self._config_cache["teacher"] = self._load_yaml_config(
                self.teacher_config_path, self._get_default_teacher_config()
            )
    ...
    def _get_default_game_config(self) -> Dict:
        ...
        return {
            ...
            "colors": {
                "white": [255, 255, 255],
                "black": [0, 0, 0],
                "flame_colors": [[255, 69, 0], [255, 215, 0], [0, 191, 255]],
            },
            "effects": {
                "laser_effects": [
                    {
            ...
            "difficulty": {
                "easy": {"spawn_interval": 90, "collision_delay": 400, "target_speed": 2},
                "medium": {"spawn_interval": 60, "collision_delay": 250, "target_speed": 3},
                "hard": {"spawn_interval": 40, "collision_delay": 150, "target_speed": 4},
            },
            "display": {
                "default_width": 1200,
                "default_height": 800,
    ...
    def _get_default_teacher_config(self) -> Dict:
        ...
        return {
            "classroom": {"teacher_name": "", "class_name": "", "school_name": ""},
            "content_customization": {
                "enabled_game_modes": ["alphabet", "numbers", "shapes", "colors"],
                "custom_sequences": {
                    "alphabet": {"enabled": True, "custom_letters": [], "focus_letters": []},
                    "numbers": {
                        "enabled": True,
                        "range_start": 1,
                        "range_end": 10,
                    ...
                    "shapes": {"enabled": True, "custom_shapes": []},
                    "colors": {"enabled": True, "custom_colors": []},
                },
            },
            "difficulty_settings": {
    ...
    def get_legacy_settings(self) -> Dict:
        ...
        try:
            return {
                ...
                "LEVEL_PROGRESS_PATH": self.get(
                    "game.game.level_progress_path", "level_progress.txt"
                ),
                "WHITE": tuple(self.get("game.colors.white", [255, 255, 255])),
                "BLACK": tuple(self.get("game.colors.black", [0, 0, 0])),
                "FLAME_COLORS": [
                    tuple(color)
                    for color in self.get(
    ...
    def get_custom_sequence(self, sequence_type: str) -> list:
        ...
        custom_config = self.get(
            f"teacher.content_customization.custom_sequences.{sequence_type}", {}
        )
        ...
        if custom_config.get("enabled", True):
            ...
            elif sequence_type in ["shapes", "colors"] and custom_config.get(
                f"custom_{sequence_type}"
            ):
    ...
    def get_difficulty_settings(self) -> Dict:
        ...
        return self.get(
            f"game.difficulty.{current_difficulty}", self.get("game.difficulty.medium", {})
        )
        ...
        try:
            ...
            if imported_config:
                self._config_cache["teacher"] = self._merge_configs(
                    self._get_default_teacher_config(), imported_config
                )
