...
class EnhancedParticle:
    ...
    def initialize(self, x: float, y: float, vx: float, vy: float, 
                  life: float, color: Tuple[int, int, int, int] = (255, 255, 255, 255),
                  size: float = 3.0, texture_name: str = None):
    ...
    def draw(self, screen: pygame.Surface, atlas_manager = None):
        ...
        if self.texture_name and atlas_manager:
            ...
            if texture:
                ...
                if self.scale != 1.0 or self.rotation != 0:
                    scaled_size = (int(texture.get_width() * self.scale), 
        ...
        if final_size > 0:
            ...
            # Draw to screen
            screen.blit(particle_surf, (int(self.x - final_size), int(self.y - final_size)),
    ...
    def update(self, dt: float):
        ...
        for particle in self.particles:
            ...
            if self.culling_enabled:
                if (particle.x < -self.culling_margin or 
                    particle.x > self.screen_width + self.culling_margin or
                    particle.y < -self.culling_margin or 
                    particle.y > self.screen_height + self.culling_margin):
                    particles_to_remove.append(particle)
...
def create_hit_effect(particle_system: EnhancedParticleSystem, x: float, y: float, 
...
def create_trail_effect(particle_system: EnhancedParticleSystem, x: float, y: float,
