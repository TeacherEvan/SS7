...
class ContextAwareVoiceGenerator:
    ...
    def generate_contextual_voice(
        ...
        self.logger.info(
            f"Selected voice '{voice_name}' for {content_type.value} in {level_type.value}"
        )

        return self._generate_with_voice(text, filename, voice_id, voice_settings, voice_name)

    def _generate_with_voice(
        self, text: str, filename: str, voice_id: str, settings: Dict, voice_name: str
    ) -> bool:
    ...
    def generate_number_voice(self, number: str) -> bool:
        ...
        return self.generate_contextual_voice(number, number, LevelType.NUMBERS, ContentType.NUMBER)

    def generate_general_voice(
        self, text: str, filename: str, level_type: LevelType = None
    ) -> bool:
        """Generate voice for general game content."""
        if level_type is None:
            level_type = LevelType.ALPHABET  # Default level type

        return self.generate_contextual_voice(text, filename, level_type, ContentType.GENERAL)
    ...
    def test_contextual_generation(self):
        ...
        test_cases = [
            ...
            (
                "Emoji 'apple' in Alphabet",
                lambda: self.generate_emoji_voice("apple", LevelType.ALPHABET),
            ),
            ...
            (
                "Shape 'circle' hit",
                lambda: self.generate_target_hit_voice("circle", LevelType.SHAPES),
            ),
