...
class DebugDisplay:
    ...
    def draw(self):
        ...
        if "gameplay" in self.stats:
            ...
            self._draw_text(
                overlay, f"Targets Missed: {gameplay.get('targets_missed', 0)}", 15, y_offset
            )
            y_offset += line_height
            ...
            self._draw_text(
                overlay, f"Current Streak: {gameplay.get('current_streak', 0)}", 15, y_offset
            )
            y_offset += line_height
        ...
        if "summaries" in self.stats:
            ...
            for tracker_name, summary in summaries.items():
                if "total_events" in summary:
                    self._draw_text(
                        overlay, f"{tracker_name}: {summary['total_events']}", 15, y_offset
                    )
                    y_offset += line_height
