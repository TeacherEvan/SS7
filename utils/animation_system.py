...
class Animation:
    ...
    def __init__(self, target_object: Any, property_name: str, start_value: float, 
                 end_value: float, duration: float, ease_type: EaseType = EaseType.LINEAR,
...
class AnimatedTarget:
    ...
    def __init__(self, x: float, y: float, text: str, color: Tuple[int, int, int] = (255, 255, 255)):
    ...
    def draw(self, screen: pygame.Surface, font: pygame.font.Font):
        ...
        if self.scale != 1.0 or self.rotation != 0:
            # Scale
            if self.scale != 1.0:
                new_size = (int(text_surface.get_width() * self.scale * self.pulse_scale),
...
class AnimationManager:
    ...
    def create_target_animation(self, target: AnimatedTarget, property_name: str, 
                               end_value: float, duration: float, 
                               ease_type: EaseType = EaseType.EASE_OUT,
                               delay: float = 0.0, callback: Optional[Callable] = None) -> Animation:
        ...
        animation = Animation(target, property_name, start_value, end_value, 
                            duration, ease_type, delay, callback)
    ...
    def animate_target_hit(self, target: AnimatedTarget):
        ...
        self.create_target_animation(target, "glow_intensity", 0.0, 0.2, EaseType.EASE_IN, delay=0.1)
        ...
        for i in range(5):  # 5 shake iterations
            ...
            self.create_target_animation(target, "shake_x", shake_amount, 0.01, EaseType.LINEAR, delay=delay)
            self.create_target_animation(target, "shake_x", -shake_amount, 0.01, EaseType.LINEAR, delay=delay + 0.01)
            if i == 4:  # Last iteration
                self.create_target_animation(target, "shake_x", 0, 0.01, EaseType.LINEAR, delay=delay + 0.02, callback=shake_callback)
    
    def animate_target_exit(self, target: AnimatedTarget, animation_type: str = "scale"):
        """Animate target exit."""
        if animation_type == "scale":
            # Scale to 0
            self.create_target_animation(target, "scale", 0.0, 0.3, EaseType.EASE_IN, callback=lambda anim: setattr(target, "visible", False))
        
        elif animation_type == "fade":
            # Fade out
            self.create_target_animation(target, "alpha", 0, 0.3, EaseType.EASE_IN, callback=lambda anim: setattr(target, "visible", False))
        
        elif animation_type == "spin_out":
            # Spin and scale out
            self.create_target_animation(target, "rotation", 720, 0.5, EaseType.EASE_IN)
            self.create_target_animation(target, "scale", 0.0, 0.5, EaseType.EASE_IN, callback=lambda anim: setattr(target, "visible", False))
    
    def create_continuous_pulse(self, target: AnimatedTarget, intensity: float = 0.1, speed: float = 2.0):
        """Create a continuous pulsing animation."""
        animation = Animation(target, "pulse_scale", 1.0, 1.0 + intensity, speed / 2, EaseType.EASE_IN_OUT)
...
def create_floating_animation(target: AnimatedTarget, amplitude: float = 10, speed: float = 3):
    ...
    original_y = target.y
    animation = Animation(target, "y", original_y, original_y - amplitude, speed / 2, EaseType.EASE_IN_OUT)
    animation.set_loop(True, reverse=True)
    target.add_animation(animation)
    animation.start()

def create_breathing_animation(target: AnimatedTarget, scale_amount: float = 0.05, speed: float = 2):
