...
class HUDManager:
    ...
    def display_info(
        ...
        **kwargs,
        ...
        if mode == "colors":
            self._display_colors_hud(
                screen, target_letter, overall_destroyed, total_letters, text_color, **kwargs
            )
        else:
            self._display_standard_hud(
                screen, target_letter, overall_destroyed, total_letters, mode, text_color
            )

    def _display_colors_hud(
        self, screen, target_letter, overall_destroyed, total_letters, text_color, **kwargs
    ):
        """Display HUD for colors mode with special layout (Score and Ability removed)."""
        # Target color at top left
        target_color_text = self.small_font.render(
            f"Target Color: {target_letter}", True, text_color
        )
        ...
        if target_dots_left is not None:
            dots_left_text = self.small_font.render(
                f"Remaining: {target_dots_left}", True, text_color
            )
        ...
        if current_color_dots_destroyed is not None:
            next_color_text = self.small_font.render(
                f"Next color in: {5 - current_color_dots_destroyed} dots", True, text_color
            )
            screen.blit(next_color_text, (self.margin, self.margin + self.line_height * 2))

        # Progress on top right
        progress_text = self.small_font.render(
            f"Destroyed: {overall_destroyed}/{total_letters}", True, text_color
        )
        progress_rect = progress_text.get_rect(topright=(self.width - self.margin, self.margin))
        screen.blit(progress_text, progress_rect)

    def _display_standard_hud(
        self, screen, target_letter, overall_destroyed, total_letters, mode, text_color
    ):
        ...
        progress_rect = progress_text.get_rect(
            topright=(self.width - self.margin, self.margin + self.line_height)
        )
        screen.blit(progress_text, progress_rect)
    ...
    def display_collision_status(
        self, screen, collision_enabled, collision_delay_counter, collision_delay_frames
    ):
        """
        ...
        if not collision_enabled:
            text_color = (
                BLACK if self.glass_shatter_manager.get_background_color() == WHITE else WHITE
            )
            countdown_text = (
                f"Collisions in: {(collision_delay_frames - collision_delay_counter) // 50}s"
            )
...
class CheckpointManager:
    ...
    def __init__(self, width, height, fonts, small_font):
        ...
        self.menu_rect = pygame.Rect(
            (self.center_x + 20, self.center_y + 50), (self.button_width, self.button_height)
        )
    ...
    def _store_colors_state(self, mode, **kwargs):
        ...
        try:
            return {
                ...
                "color_sequence": (
                    getattr(__main__, "color_sequence", []).copy()
                    if hasattr(__main__, "color_sequence")
                    else None
                ),
                "next_color_index": getattr(__main__, "next_color_index", 0),
                "target_dots_left": getattr(__main__, "target_dots_left", 10),
                "used_colors": (
                    getattr(__main__, "used_colors", []).copy()
                    if hasattr(__main__, "used_colors")
                    else []
                ),
            }
        except:
            ...
            return {
                ...
                "color_sequence": (
                    kwargs.get("color_sequence", []).copy()
                    if kwargs.get("color_sequence")
                    else None
                ),
                "next_color_index": kwargs.get("next_color_index", 0),
                "target_dots_left": kwargs.get("target_dots_left", 10),
                "used_colors": (
                    kwargs.get("used_colors", []).copy() if kwargs.get("used_colors") else []
                ),
            }
    ...
    def _draw_neon_button(self, screen, rect, color):
        ...
        for i in range(5):
            glow_rect = pygame.Rect(
                rect.x - i * 2, rect.y - i * 2, rect.width + i * 4, rect.height + i * 4
            )
...
class FlamethrowerManager:
    ...
    def create_flamethrower(
        self, start_x, start_y, end_x, end_y, colors=None, widths=None, duration=10
    ):
        """
        ...
        for i in range(num_circles):
            ...
            if glow_radius > 0:
                ...
                if glow_surface is None:
                    ...
                    pygame.draw.circle(
                        glow_surface, (*color, 100), (glow_radius, glow_radius), glow_radius
                    )
                    self.glow_cache[cache_key] = glow_surface

                screen.blit(
                    glow_surface,
                    (circle_x + jitter_x - glow_radius, circle_y + jitter_y - glow_radius),
                )

            # Draw main flame circle
            if radius > 0:
                pygame.draw.circle(
                    screen, color, (circle_x + jitter_x, circle_y + jitter_y), radius
                )
...
class CenterPieceManager:
            ...
            self.swirl_particles.append(
                {
                    ...
                    "rotation_speed": random.uniform(0.02, 0.04)
    ...
    def _update_swirl_particles(self):
        ...
        for particle in self.swirl_particles:
            if self.particles_converging and self.convergence_target:
                ...
                pulse = (
                    math.sin(
                        current_time_ms * 0.001 * particle["pulse_speed"] + particle["pulse_offset"]
                    )
                    * 20
                )  # Pulse magnitude
        ...
        if self.particles_converging:
            ...
            if (
                self.convergence_timer <= 0 or not self.swirl_particles
            ):  # Stop if timer runs out or no particles left
                self.particles_converging = False
                self.convergence_target = None
    ...
    def _get_interpolated_color(self):
        ...
        r = int(
            self.player_current_color[0] * (1 - self.player_color_transition)
            + self.player_next_color[0] * self.player_color_transition
        )
        g = int(
            self.player_current_color[1] * (1 - self.player_color_transition)
            + self.player_next_color[1] * self.player_color_transition
        )
        b = int(
            self.player_current_color[2] * (1 - self.player_color_transition)
    ...
    def _draw_text_target_cached(self, screen, target_letter, mode, center_x, center_y):
        ...
        if self.resource_manager:
            try:
                cached_surface = self.resource_manager.get_center_target_surface(
                    mode, target_letter, center_target_color
                )
        ...
        self._draw_text_target_fallback(
            screen, target_letter, mode, center_x, center_y, center_target_color
        )

    def _draw_text_target_fallback(
        self, screen, target_letter, mode, center_x, center_y, center_target_color
    ):
    ...
    def _draw_shape_target(self, screen, target_letter, center_x, center_y):
        ...
        if value == "Rectangle":
            rect = pygame.Rect(
                pos[0] - int(size * 1.5) // 2, pos[1] - size // 2, int(size * 1.5), size
            )
            ...
            for i in range(5):
                ...
                points.append(
                    (pos[0] + r_size * math.cos(angle), pos[1] + r_size * math.sin(angle))
                )
            pygame.draw.polygon(screen, center_target_color, points, 8)
...
class SoundManager:
    ...
    def _initialize_mixer(self):
        ...
        try:
            ...
            if pygame.mixer.get_init():
                ...
                if self.event_tracker:
                    self.event_tracker.track_initialization(
                        "sound_mixer", True, "Using existing mixer"
                    )
                return
            ...
            if self.event_tracker:
                self.event_tracker.track_initialization(
                    "sound_mixer", True, "New mixer initialized"
                )
    ...
    def _load_default_sounds(self):
        ...
        sound_files = {"explosion": "explosion.wav", "laser": "laser.wav"}
