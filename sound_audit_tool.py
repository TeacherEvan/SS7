...
class SoundAuditor:
        ...
        for filename in expected_files:
            ...
            if file_path.exists():
                ...
                try:
                    ...
                    if duration < 0.5:
                        category_data["quality_issues"].append(f"{filename}: Too short ({duration:.2f}s)")
                    elif duration > 2.0:
                        category_data["quality_issues"].append(f"{filename}: Too long ({duration:.2f}s)")
                    
                    if file_size < 30000:  # Less than 30KB
                        category_data["quality_issues"].append(f"{filename}: Low quality (file too small)")
    ...
    def _calculate_overall_metrics(self):
        ...
        if all_durations:
            self.results["quality_metrics"] = {
                ...
                "duration_consistency": "Good" if statistics.stdev(all_durations) < 0.3 else "Inconsistent",
                "average_file_size": f"{statistics.mean(all_sizes):.1f} KB",
                "total_quality_issues": total_issues,
                "overall_quality_score": max(0, 100 - (total_issues * 5))  # Deduct 5 points per issue
            }
    ...
    def _generate_recommendations(self):
        ...
        missing_count = sum(len(data["missing_files"]) for data in self.results["categories"].values())
        if missing_count > 0:
        ...
        # Check for quality issues
        total_quality_issues = sum(len(data["quality_issues"]) for data in self.results["categories"].values())
        if total_quality_issues > 0:
            recommendations.append(f"🔧 {total_quality_issues} files have quality issues that need attention")
        
        # Educational recommendations
        if self.results["quality_metrics"].get("overall_quality_score", 0) < 80:
            recommendations.append("📚 Consider regenerating low-quality pronunciation files for better learning experience")
        
        recommendations.append("✅ Use consistent pronunciation timing (0.7-1.5s) for educational effectiveness")
    ...
    def print_detailed_report(self):
        ...
        print(f"   Overall Quality Score: {self.results['quality_metrics'].get('overall_quality_score', 0)}/100")
        ...
        for category, data in self.results["categories"].items():
            ...
            if "duration_stats" in data:
                ...
                print(f"      ⏱️ Duration: {stats['min']}s - {stats['max']}s (avg: {stats['mean']}s)")
