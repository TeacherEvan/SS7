    ...
    scenarios = [
        ...
        },
        {
            ...
            "tests": [
                ...
                ("Color: yellow", lambda: cvg.generate_target_hit_voice("yellow", LevelType.COLORS)),
            ]
        },
        {
            "title": "⭕ Shapes Level - Random Voice Selection (40% Male, 60% Female)",
            "tests": [
                ("Shape: circle", lambda: cvg.generate_target_hit_voice("circle", LevelType.SHAPES)),
                ("Shape: square", lambda: cvg.generate_target_hit_voice("square", LevelType.SHAPES)),
                ("Shape: triangle", lambda: cvg.generate_target_hit_voice("triangle", LevelType.SHAPES)),
            ]
        },
        {
    ...
    for scenario in scenarios:
        ...
        for test_name, test_func in scenario['tests']:
            ...
            if success:
                ...
                if "emoji" in test_name.lower() and ("alphabet" in scenario['title'].lower() or "case" in scenario['title'].lower()):
                    ...
                    if "red" in test_name.lower():
                        voice_key, _ = vas.get_voice_for_content(LevelType.COLORS, ContentType.TARGET_HIT, "red", "target_destroyed")
                    elif "blue" in test_name.lower():
                        voice_key, _ = vas.get_voice_for_content(LevelType.COLORS, ContentType.TARGET_HIT, "blue", "target_destroyed")
                    elif "green" in test_name.lower():
                        voice_key, _ = vas.get_voice_for_content(LevelType.COLORS, ContentType.TARGET_HIT, "green", "target_destroyed")
                    elif "yellow" in test_name.lower():
                        voice_key, _ = vas.get_voice_for_content(LevelType.COLORS, ContentType.TARGET_HIT, "yellow", "target_destroyed")
                    elif "circle" in test_name.lower():
                        voice_key, _ = vas.get_voice_for_content(LevelType.SHAPES, ContentType.TARGET_HIT, "circle", "target_destroyed")
                    elif "square" in test_name.lower():
                        voice_key, _ = vas.get_voice_for_content(LevelType.SHAPES, ContentType.TARGET_HIT, "square", "target_destroyed")
                    elif "triangle" in test_name.lower():
                        voice_key, _ = vas.get_voice_for_content(LevelType.SHAPES, ContentType.TARGET_HIT, "triangle", "target_destroyed")
                    
                    selected_voice = "🗣️ Male (Elderly Stoic)" if voice_key == "male_elderly_stoic" else "🗣️ Female (Rachel)"
                else:
